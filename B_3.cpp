...
private:
	struct Node {
		...
		Node* parent;
		size_t sum = 0;
		Node(T Value) { value = Value; sum = 0; left = nullptr; right = nullptr; parent = nullptr; }
	};

	Node* root_ = nullptr;
	Node* search(Node* node, T value);
	size_t getSum(Node* node);
	...
	Node* next(T x, Node* current);
	Node* prev(T x, Node* current);
	std::pair <Node*, Node*> split(T x);
	Node* insert(T x, Node* root);
	void erase(T x);
...
typename SplayTree<T, Comparator>::Node* SplayTree<T, Comparator>::search(Node* node, T value) {
	if (node == nullptr) {
		return nullptr;
	}
	if (value == node->value) {
...
size_t SplayTree<T, Comparator>::getSum(Node* node) {
...
typename SplayTree<T, Comparator>::Node* SplayTree<T, Comparator>::getMax(Node* node) {
	while (node->right) {
		node = node->right;
	}
	return node;
...
typename SplayTree<T, Comparator>::Node* SplayTree<T, Comparator>::next(T x, Node* current) {
...
void SplayTree<T, Comparator>::next(T x) {
...
typename SplayTree<T, Comparator>::Node* SplayTree<T, Comparator>::prev(T x, Node* current) {
...
void SplayTree<T, Comparator>::prev(T x) {
...
std::pair <typename SplayTree<T, Comparator>::Node*, typename SplayTree<T, Comparator>::Node*> SplayTree<T, Comparator>::split(T x) {
	if (!root_) {
		return std::make_pair(root_, nullptr);
	}

	if (!Comparator(x, getMax(root_)->value)) {
		return std::make_pair(root_, nullptr);
	}
...
typename SplayTree<T, Comparator>::Node* SplayTree<T, Comparator>::insert(T x, Node* root) {
...
void SplayTree<T, Comparator>::insert(T x) {
	if (!search(root_, x)) {
...
void SplayTree<T, Comparator>::erase(T x) {
	if (search(root_, x)) {
...
T SplayTree<T, Comparator>::sum(T l, T r) {
	T result;
	if (!root_) {
		result = 0;
		std::cout << 0;
